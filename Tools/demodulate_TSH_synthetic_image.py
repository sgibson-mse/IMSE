...
def demodulate_image(image):
    ...
    mask = box_filter(image, x_size=int(len(image)), y_size=60, centre=[int(len(image)/2),433]) #box_filter(shift_image, x_size = int(len(image)), y_size=35, centre=[int(len(image)/2),23])
    dc_mask = box_filter(image, x_size=int(len(image)), y_size=60, centre=[int(len(image)/2), int(len(image)/2)])
