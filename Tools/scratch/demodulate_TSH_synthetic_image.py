...
def demodulate_image(image):
    ...
    mask = box_filter(image, x_size=int(len(image)), y_size=50, centre=[int(len(image)/2),433]) #box_filter(shift_image, x_size = int(len(image)), y_size=35, centre=[int(len(image)/2),23])
    dc_mask = box_filter(image, x_size=int(len(image)), y_size=50, centre=[int(len(image)/2), int(len(image)/2)])
...
def msesim_profiles(npx,data):
    # Load input from msesim
    stokes = data["total_stokes"]
    ...
    xyz0 = data["central_coordinates"]
    c_xyz = data["collection_lens_coordinates"]

    xyz_grid = xyz0.reshape(npx,npx,3)

    R_vector = data["resolution_vector(R)"]
...
def demodulate_fw_images():
    ...
    image_1_fw = load_image(filename='/home/sgibson/PycharmProjects/IMSE/Model/edgecurrent1.hdf')
    image_2_fw = load_image(filename='/home/sgibson/PycharmProjects/IMSE/Model/edgecurrent2.hdf')
