...
class Crystal(object):
    ...
    def __init__(self, wavelength, thickness, cut_angle, name, nx, ny, pixel_size, orientation, two_dimensional):
        ...
        self.wavelength = wavelength * 10 ** 6
        self.cut_angle = cut_angle * (np.pi/180.) #optical axis in radians
        self.L = thickness * 10**6
        self.focal_length = 0.085 *10**6
        self.angular_frequency = 2 * np.pi * (constants.c / self.wavelength) #in micrometers

        x = np.linspace(-camera.sensor_size/2, camera.sensor_size/2, nx)
        y = np.linspace(-camera.sensor_size/2, camera.sensor_size/2, ny)
    ...
    def non_axial_rays(self):
        ...
        self.alpha = np.arctan(np.sqrt((self.xx)**2 + (self.yy)**2)/(self.focal_length*10**-6))
    ...
    def phase_total(self):
        self.phi = ((2*np.pi*self.L*10**-6)/(self.wavelength*10**-6)) * ( (self.no**2 - constants.n_air**2*np.sin(self.alpha)**2)**0.5 + (constants.n_air*(self.no**2 - self.ne**2)*np.sin(self.cut_angle)*np.cos(self.cut_angle)*np.cos(self.delta)*np.sin(self.alpha))/(self.ne**2*np.sin(self.cut_angle)**2 + self.no**2*np.cos(self.cut_angle)**2) + ((-1*self.no * (self.ne**2 * (self.ne**2 * np.sin(self.cut_angle)**2 + (self.no**2 * np.cos(self.cut_angle)**2)) - (self.ne**2 - (self.ne**2 - self.no**2)*np.cos(self.cut_angle)**2 * np.sin(self.delta)**2)*constants.n_air**2*np.sin(self.alpha)**2)**0.5)/(self.ne**2*np.sin(self.cut_angle)**2 + self.no**2*np.cos(self.cut_angle)**2)))
        return self.phi
    ...
    def phase_shear(self):
        self.phi_shear = (2 * np.pi * self.L * 10 ** -6 * self.birefringence * np.sin(2 * self.cut_angle)) / (2 * self.wavelength * 10 ** -6 * ((self.ne + self.no) / 2) * self.focal_length * 10 ** -6) #*self.yy
