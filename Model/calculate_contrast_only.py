...
def angle_of_incidence():
    ...
    pixel_x = np.linspace(-10.24*10**-3,10.24*10**-3,21)
...
def find_optimal_crystal(wavelength_vector, stokes_full, channel_full, delay_thickness, delay_cut_angle, displacer_thickness, displacer_cut_angle):
...
def design_filter(wavelength, alpha, tilt_angle, scale_fwhm, lambda_0):
    """
    Design a filter to capture the whole MSE multiplet, and account for the doppler shift variation in wavelength.
    :return: Transmittance through filter as a function of wavelength, wavelengths through the filter (for a given angle of incidence)
    """

    #load in 3 cavity filter
    ...
    # Wavelengths through filter for given angle of incidence

    print('alpha', alpha, 'tilt', tilt_angle)
    ...
    for i in range(len(lambdas)):
        ...
        interp_t = interp1d(lambda_values, cavity_filter)(wavelength)
        interp_transmittance.append(interp_t)

    interp_trans = np.array([interp_transmittance])[0, :, :]

    return interp_transmittance
