...
class Camera(object):
    ...
    def flir_specs(self, name):
        ...
        self.sensor_size_x = 14.4*10**-3
        self.sensor_size_y = 9.9*10**-3

        self.ADC = 12 #12-bit

        self.read_noise = 4.69 #electrons in HCG
        self.dark_noise = 0 #electrons, not specified in spec sheet?

        self.fill_factor = 1 # how much of the pixel is actually available for light collection
        self.fullwell_capacity = 19557 #electrons in high conversion gain mode
        self.fullwell_lcg = 98654 #electrons in low conversion gain mode
        self.pixel_size = 9*10**-6

        self.gain = 0.2093 #electrons/count -> 12 bit ADC taking FWC 19557/4095 -> 0.21e/ADU
        self.quantum_efficiency = 0.54
        self.integration_time = np.arange(5*10**-3, 20.5*10**-3, 0.5*10**-3) #196 fps limit, so 5ms resolution limit.
    ...
    def grasshopper_specs(self, name):
        ...
        self.sensor_size_x = 11.25 * 10 ** -3
        self.sensor_size_y = 7.03 * 10 ** -3

        self.ADC = 12  # 12-bit

        self.read_noise = 7  # electrons in HCG
        self.dark_noise = 0  # electrons, not specified in spec sheet?

        self.fill_factor = 1  # how much of the pixel is actually available for light collection
        self.fullwell_capacity = 32000 # electrons in high conversion gain mode
        self.pixel_size = 5.86 * 10 **-6

        self.gain = 0.52 # electrons/count -> 12 bit ADC taking FWC 19557/4095 -> 0.21e/ADU
        self.quantum_efficiency = 0.50
        self.integration_time = np.arange(6.5 * 10 ** -3, 20.5 * 10 ** -3,
                                          0.5 * 10 ** -3)  # 163 fps limit, so 6.1ms resolution limit.
