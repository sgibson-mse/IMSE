...
def demodulate(timeslices):
    ...
    gamma = np.zeros((1760,1960, len(timeslices)))

    for i, timeslices in enumerate(timeslices):

        filename = '/home/sam/Desktop/Projects/AUG/IMSE/data/3534/gmds/RAW/IMAGE_{}.nc'.format(timeslices)
        ...
        phi_dfilter = box_filter(image, x_size=100, y_size=100, centre=[1134,828])

        phid_plus_phi_s = box_filter(image, x_size = 100, y_size=100, centre= [1187, 969])

        phid_minus_phi_s = box_filter(image, x_size =100, y_size=100, centre= [1081,694])
...
background = '/home/sam/Desktop/Projects/AUG/IMSE/data/3534/35346/RAW/IMAGE_0.nc'

timeslices = np.arange(100,150,1)
